...
interface ProductoDetalleMarbete {
...
const productoDetalleMarbetes = ref<ProductoDetalleMarbete[]>([]);
const filteredMarbetes = ref<ProductoDetalleMarbete[]>([]);
...
const loadProductoDetalleMarbetes = async () => {
  if (!selectedPeriodo.value) {
    productoDetalleMarbetes.value = [];
  ...
  try {
    ...
    const response = await axiosConfiguration.doPost('/labels/reports/product-detail', body);
    ...
    productoDetalleMarbetes.value = content.map((item: any) => ({
      ...
      existencias: item.existencias !== null && item.existencias !== undefined ? Number(item.existencias) : null,
      total: item.total !== null && item.total !== undefined ? Number(item.total) : null,
      fuenteConteo: String(item.fuenteConteo ?? 'SIN_CONTEO')
    }));
    applySearchFilterProductoDetalle();
    LoadAlert(false);
  } catch (error) {
    LoadAlert(false);
    ToastError('Error', 'No se pudo cargar el detalle de producto');
    productoDetalleMarbetes.value = [];
    filteredMarbetes.value = [];
  } finally {
    loading.value = false;
...
const applySearchFilterProductoDetalle = () => {
  let filtered = [...productoDetalleMarbetes.value];
...
const calcularEstadisticas = () => {
  estadisticas.value.totalConDiferencia = productoDetalleMarbetes.value.length;
};
...
const exportToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/product-detail/pdf',
    periodId: selectedPeriodo.value.id,
    warehouseId: selectedAlmacen.value?.id === 0 ? null : selectedAlmacen.value?.id,
    fileName: `inventario-fisico-por-producto-detalle-${selectedPeriodo.value.id}.pdf`,
    onStart: () => {
...
const exportAllToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/product-detail/all/pdf',
    periodId: selectedPeriodo.value.id,
    fileName: `inventario-fisico-por-producto-detalle-todos-${selectedPeriodo.value.id}.pdf`,
  ...
  <div class="reporte-container">
    <div class="header-section">
      <div class="title-wrapper">
        ...
        <p class="subtitle">
          Proporciona un desglose detallado de los marbetes por producto, permitiendo analizar el inventario a nivel de artículo.
        </p>
      </div>
    </div>
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p>Cargando detalle de productos...</p>
    </div>
    <div v-else-if="filteredMarbetes.length > 0" class="table-section">
      <table class="data-table">
        ...
        <tbody>
        <tr v-for="item in filteredMarbetes" :key="item.numeroMarbete">
          <td><span class="folio-badge">{{ item.numeroMarbete }}</span></td>
          <td>
            <div class="product-info">
              <strong>{{ item.descripcionProducto }}</strong>
            </div>
          </td>
          ...
          <td class="text-center">
            <span :class="['number-badge', item.fuenteConteo !== 'C2' ? 'warn-badge' : '']">
              {{ formatNumber(item.existencias) }}
            </span>
          </td>
          <td class="text-center"><span class="number-badge">{{ formatNumber(item.total) }}</span></td>
          <td class="text-center">
            <span :class="['fuente-badge', `fuente-${item.fuenteConteo.toLowerCase().replace('_','-')}`]">
              {{ item.fuenteConteo }}
            </span>
          </td>
        </tr>
    ...
    <div v-else class="empty-state">
      ...
      <p v-if="!selectedPeriodo">Selecciona un período para ver el detalle de productos.</p>
      <p v-else>No hay datos para este período y almacén.</p>
    </div>
  </div>
</template>

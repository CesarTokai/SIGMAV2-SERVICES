...
const loadMarbetesConDiferencia = async () => {
  ...
  try {
    ...
    const response = await axiosConfiguration.doPost('/labels/reports/with-differences', body);
...
const exportToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/with-differences/pdf',
    periodId: selectedPeriodo.value.id,
    warehouseId: selectedAlmacen.value?.id === 0 ? null : selectedAlmacen.value?.id,
    fileName: `Marbetes-con-diferencias-${selectedPeriodo.value.id}.pdf`,
...
const exportAllToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/with-differences/all/pdf',
    periodId: selectedPeriodo.value.id,
    fileName: `Marbetes-diferencias-todos-${selectedPeriodo.value.id}.pdf`,
  ...
  <div class="reporte-container">
    <div class="header-section">
      <div class="title-wrapper">
        ...
        <p class="subtitle">
          Muestra los marbetes donde existen diferencias entre el conteo físico y el sistema.
        </p>
      </div>
    </div>
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p>Cargando marbetes con diferencia...</p>
    </div>
    ...
    <div v-else-if="filteredMarbetes.length > 0" class="table-section">
      <table class="data-table">
        ...
        <tr v-for="marbete in filteredMarbetes" :key="marbete.numeroMarbete">
          <td>
            <span class="folio-badge">{{ marbete.numeroMarbete }}</span>
          </td>
          <td>
          ...
          <td class="text-center">
            <span v-if="marbete.diferencia !== null" :class="['diferencia-badge', marbete.diferencia > 0 ? 'positivo' : marbete.diferencia < 0 ? 'negativo' : 'igual']">
              {{ marbete.diferencia > 0 ? '+' : '' }}{{ formatNumber(marbete.diferencia) }}
            </span>
            <span v-else class="diferencia-badge igual">-</span>
          </td>
          <td>
            <span v-if="marbete.estado === 'CANCELADO'" class="status-badge status-cancelled">
    ...
    <div v-else class="empty-state">
      ...
      <p v-if="!selectedPeriodo">
        Selecciona un período para ver los marbetes con diferencia.
      </p>
      <p v-else>
        No hay marbetes con diferencia en este período y almacén.
      </p>
    </div>
  </div>
</template>

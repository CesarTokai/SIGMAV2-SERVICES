...
interface AlmacenDetalleMarbete {
  ...
  estado: string;
  cancelado: boolean;
  fuenteConteo: string; // "C2" | "C1" | "SIN_CONTEO"
}

interface AlmacenProductoAgrupado {
  claveAlmacen: string;
  nombreAlmacen: string;
  claveProducto: string;
  descripcionProducto: string;
  unidad: string;
  cantidad: number;
  marbetes: AlmacenDetalleMarbete[];
  cancelado: boolean;
}
...
const almacenDetalleMarbetes = ref<AlmacenDetalleMarbete[]>([]);
const almacenProductoAgrupado = ref<AlmacenProductoAgrupado[]>([]);
const filteredMarbetes = ref<AlmacenProductoAgrupado[]>([]);
...
const loadAlmacenDetalleMarbetes = async () => {
  if (!selectedPeriodo.value) {
    almacenDetalleMarbetes.value = [];
    almacenProductoAgrupado.value = [];
  ...
  try {
    ...
    const response = await axiosConfiguration.doPost('/labels/reports/warehouse-detail', body);
    ...
    almacenDetalleMarbetes.value = content.map((item: any) => ({
      ...
      cantidad: item.cantidad !== null && item.cantidad !== undefined ? Number(item.cantidad) : null,
      estado: String(item.estado ?? ''),
      cancelado: Boolean(item.cancelado ?? false),
      fuenteConteo: String(item.fuenteConteo ?? 'SIN_CONTEO')
    }));

    // Agrupar datos por almacén y producto
    almacenProductoAgrupado.value = groupByAlmacenAndProduct(almacenDetalleMarbetes.value);

    applySearchFilterAlmacenDetalle();
    LoadAlert(false);
  } catch (error) {
    LoadAlert(false);
    ToastError('Error', 'No se pudo cargar el detalle de almacén');
    almacenDetalleMarbetes.value = [];
    almacenProductoAgrupado.value = [];
    filteredMarbetes.value = [];
  } finally {
...
// Agrupar marbetes por almacén y producto
const groupByAlmacenAndProduct = (marbetes: AlmacenDetalleMarbete[]): AlmacenProductoAgrupado[] => {
  const grouped = new Map<string, AlmacenProductoAgrupado>();

  marbetes.forEach(marbete => {
    const key = `${marbete.claveAlmacen}-${marbete.claveProducto}`;

    if (!grouped.has(key)) {
      grouped.set(key, {
        claveAlmacen: marbete.claveAlmacen,
        nombreAlmacen: marbete.nombreAlmacen,
        claveProducto: marbete.claveProducto,
        descripcionProducto: marbete.descripcionProducto,
        unidad: marbete.unidad,
        cantidad: marbete.cantidad ?? 0,
        marbetes: [],
    ...
    group.marbetes.push(marbete);
    // Si es una cantidad válida, actualizar
    if (marbete.cantidad !== null && marbete.cantidad > 0) {
      group.cantidad = marbete.cantidad;
    }
  });
...
const applySearchFilterAlmacenDetalle = () => {
  let filtered = [...almacenProductoAgrupado.value];
...
const calcularEstadisticas = () => {
  estadisticas.value.totalConDiferencia = almacenDetalleMarbetes.value.length;
};


// Formatear fecha de manera robusta (maneja 'YYYY-MM-DD' y formatos ISO, siempre local)
...
const formatDateTime = (datetime: string | undefined): string => {
  ...
  // Si es 'YYYY-MM-DD' puro, solo fecha
  ...
  if (/^\d{4}-\d{2}-\d{2}T/.test(datetime)) {
    ...
    return dateObj.toLocaleString('es-ES', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
  }
  // Fallback
  try {
    const dateObj = new Date(datetime);
...
const exportToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/warehouse-detail/pdf',
    periodId: selectedPeriodo.value.id,
    warehouseId: selectedAlmacen.value.id,
    fileName: `almacen-detalle-${selectedAlmacen.value.clave}-${selectedPeriodo.value.id}.pdf`,
...
const exportAllToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/warehouse-detail/all/pdf',
    periodId: selectedPeriodo.value.id,
    fileName: `almacen-detalle-todos-${selectedPeriodo.value.id}.pdf`,
  ...
  <div class="reporte-container">
    ...
    <div class="header-section">
      <div class="title-wrapper">
        <h1 class="page-title">Almacén con Detalle</h1>
        <p class="subtitle">Detalla la información de marbetes por almacén, mostrando movimientos, conteos y estados específicos.</p>
      </div>
    </div>
    ...
    <div v-else-if="filteredMarbetes.length > 0" class="table-section">
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="item in filteredMarbetes" :key="item.claveAlmacen + '-' + item.claveProducto">
            <td><span class="warehouse-key">{{ item.claveAlmacen }}</span> {{ item.nombreAlmacen }}</td>
            <td><span class="clave-badge">{{ item.claveProducto }}</span></td>
            <td>{{ item.descripcionProducto }}</td>
            <td>{{ item.unidad }}</td>
            <td class="text-center"><span class="number-badge">{{ item.cantidad }}</span></td>
            <td>
              <div class="marbetes-list">
                <span v-for="marbete in item.marbetes" :key="marbete.numeroMarbete" class="folio-badge">
                  {{ marbete.numeroMarbete }}
                </span>
              </div>
            </td>
            <td class="text-center">
              <span :class="['number-badge', item.marbetes[0]?.fuenteConteo !== 'C2' ? 'warn-badge' : '']">
                {{ item.marbetes[0]?.cantidad !== null ? formatNumber(item.marbetes[0]?.cantidad || 0) : '-' }}
              </span>
            </td>
            <td>
              <span v-if="item.cancelado" class="status-badge status-cancelled">
    ...
    <div v-else class="empty-state">
      ...
      <p v-if="!selectedPeriodo">Selecciona un período para ver los datos.</p>
      <p v-else>No hay registros para este período y almacén.</p>
    </div>
...
.status-badge.status-printed .status-dot {
...
.status-badge.status-default .status-dot {

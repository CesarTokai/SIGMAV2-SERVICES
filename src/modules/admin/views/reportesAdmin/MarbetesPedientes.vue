...
const loadMarbetesConDiferencia = async () => {
  ...
  try {
    ...
    const response = await axiosConfiguration.doPost('/labels/reports/pending', body);
...
const getNormalizedEstado = (estado: string): string => {
  return estado
    .toUpperCase()
    .trim()
    .replace(/\s+/g, '');
...
// Verificar si es cancelado
const isCancelado = (estado: string): boolean => {
  return getNormalizedEstado(estado).includes('CANCELADO');
};

// Verificar si es impreso
const isImpreso = (estado: string): boolean => {
  return getNormalizedEstado(estado).includes('IMPRESO');
};
...
const exportToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/pending/pdf',
    periodId: selectedPeriodo.value.id,
    warehouseId: selectedAlmacen.value?.id === 0 ? null : selectedAlmacen.value?.id,
    fileName: `marbetes-pendientes-${selectedPeriodo.value.id}.pdf`,
...
const exportAllToPDF = async () => {
  ...
  await exportReportToPDF({
    endpoint: '/labels/reports/pending/pdf/all',
    periodId: selectedPeriodo.value.id,
    fileName: `marbetes-pendientes-todos-${selectedPeriodo.value.id}.pdf`,
  ...
  <div class="reporte-container">
    <div class="header-section">
      <div class="title-wrapper">
        <h1 class="page-title">Marbetes Pendientes</h1>
        <p class="subtitle">Reporta los marbetes que aún están pendientes de conteo o captura, ayudando a identificar tareas incompletas en el proceso de inventario.</p>
      </div>
    </div>
    ...
    <div v-if="loading" class="loading-container">
      ...
      <p>Cargando marbetes pendientes...</p>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="marbete in filteredMarbetes" :key="marbete.numeroMarbete">
            ...
            <td><div class="product-info"><strong>{{ marbete.descripcionProducto }}</strong><small>{{ marbete.claveProducto }} - {{ marbete.unidad }}</small></div></td>
            <td><div class="warehouse-info"><span class="warehouse-key">{{ marbete.claveAlmacen }}</span><small>{{ marbete.nombreAlmacen }}</small></div></td>
            ...
            <td>
              <span v-if="isCancelado(marbete.estado)" class="status-badge status-cancelled">
                {{ marbete.estado }}
              </span>
              <span v-else class="status-badge" :class="isImpreso(marbete.estado) ? 'status-printed' : 'status-default'">
    ...
    <div v-else class="empty-state">
      ...
      <p v-if="!selectedPeriodo">Selecciona un período para ver los marbetes pendientes.</p>
      <p v-else>No hay marbetes pendientes en este período y almacén.</p>
    </div>
  </div>
</template>

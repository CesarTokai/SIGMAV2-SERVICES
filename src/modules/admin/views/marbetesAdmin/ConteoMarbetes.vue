...
const buscarMarbetePorFolio = async () => {
  ...
  try {
    ...
    LoadAlert(false);
      ...
      // Mapear la respuesta al formato esperado - EXACTAMENTE como devuelve el API
       const marbete: MarbeteConteo = {
         id: response.data.id,
         folio: response.data.folio ?? parseInt(raw),
         periodId: response.data.periodId ?? selectedPeriodo.value.id,
         warehouseId: response.data.warehouseId ?? selectedAlmacen.value.id,
...
const guardarConteo = async () => {
  ...
  // ── Limpiar UI inmediatamente para respuesta rápida ──────────────────
  limpiarFormulario();

...
const loadAlmacenes = async () => {
  try {
    ...
    // Solo seleccionar primer almacén si NO hay uno seleccionado del store
...
<template>
  <div class="impresion-marbetes">
    ...
    <div class="title-section">
        ...
        <p class="subtitle">Generación e impresión de marbetes</p>
      </div>
      <div class="filters-wrapper">
        ...
        <div class="searchbar-group">
          ...
          <div v-else-if="selectedPeriodo && selectedAlmacen && !loadingPendingCount" class="help-message help-message-info">
            <span class="help-icon">ℹ️</span>
            <span class="help-text">
              <strong>Sin pendientes:</strong> No hay marbetes pendientes de impresión. Genera marbetes primero.
            </span>
          </div>
      ...
      <div class="pdf-viewer-container">
        ...
        <div class="pdf-list">
          <div class="pdf-list-header">
            ...
            <small>{{ pdfsGenerados.length }} archivo(s)</small>
          </div>
          <div
            v-for="pdf in pdfsGenerados"
            :key="pdf.id"
            :class="['pdf-item', { active: pdfSeleccionado?.id === pdf.id }]"
            @click="seleccionarPdf(pdf)"
          >
            <div class="pdf-item-info">
              <div class="pdf-item-header">
                <span class="pdf-icon">📄</span>
                <span class="pdf-name" :title="pdf.nombre">{{ pdf.nombre }}</span>
              </div>
              <div class="pdf-item-details">
                <small class="pdf-folios">
                  <span class="detail-icon">📋</span>
                  {{ pdf.folioFin }} marbetes
                </small>
                <small class="pdf-fecha">
                  <span class="detail-icon">🕒</span>
                  {{ formatDateTime(pdf.fechaGeneracion) }}
                </small>
              </div>
            </div>
            <div class="pdf-item-actions" @click.stop>
              <button class="action-btn" @click="abrirPdfEnNuevaTab(pdf)" title="Abrir en nueva pestaña">
                <span>🔗</span>
              </button>
              <button class="action-btn" @click="descargarPdf(pdf)" title="Descargar">
                <span>💾</span>
              </button>
              <button class="action-btn" @click="imprimirPdf(pdf)" title="Imprimir">
                <span>🖨️</span>
              </button>
              <button class="action-btn delete" @click="eliminarPdf(pdf)" title="Eliminar">
                <span>🗑️</span>
              </button>
            </div>
          </div>
          ...
          <div v-if="pdfSeleccionado" class="pdf-preview-content">
            <div class="pdf-preview-header">
              ...
              <div class="pdf-preview-actions">
                <button class="btn btn-sm btn-secondary" @click="abrirPdfEnNuevaTab(pdfSeleccionado)">
                  <span class="icon">🔗</span>
                  Nueva Pestaña
                </button>
                <button class="btn btn-sm btn-success" @click="descargarPdf(pdfSeleccionado)">
                  <span class="icon">💾</span>
                  Descargar
                </button>
                <button class="btn btn-sm btn-primary" @click="imprimirPdf(pdfSeleccionado)">
                  <span class="icon">🖨️</span>
                  Imprimir
                </button>
              </div>
...
.pdf-item.active {
  background: linear-gradient(135deg, #e6f7ed 0%, #d4f4e0 100%);
  border-color: #28a745;

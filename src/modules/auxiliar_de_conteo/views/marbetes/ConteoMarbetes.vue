...
const buscarMarbetePorFolio = async () => {
  ...
  try {
    ...
    const marbete: MarbeteConteo = {
      folio:               Number(response.data.folio ?? parseInt(raw)),
    ...
    ToastError('Error al buscar folio', error?.response?.data?.message || 'No se pudo buscar el folio');
...
const conteo1Deshabilitado = computed<boolean>(() => {
  // ✅ PERMITIR EDITAR C1 SIEMPRE: Solo deshabilitar si no hay marbete seleccionado
  ...
  <div class="conteo-marbetes">
        ...
        <div class="info-column">
          ...
          <div class="info-field">
            ...
            <div class="diferencia-display" :class="getDiferenciaClass()">
              ...
              <span v-else>{{ marbeteActual?.diferencia || '-' }}</span>
            </div>
          </div>
          <div class="info-field">
            <label>Estado:</label>
          ...
          <div class="info-field">
            <label>Impreso:</label>
            <span :class="['estado-badge', marbeteActual?.impreso ? 'impreso' : 'no-impreso']">
              {{ marbeteActual?.impreso ? 'Sí' : 'No' }}
            </span>
          </div>
          <div class="info-field">
            <label>Cancelado:</label>
            <span :class="['estado-badge', marbeteActual?.cancelado ? 'cancelado' : 'activo']">
    ...
    <div class="comentario-section">
      ...
      <textarea
          ...
          placeholder="Ingresa o actualiza un comentario (máx. 600 caracteres)"
          maxlength="600"
          rows="3"
      />

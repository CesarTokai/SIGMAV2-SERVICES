...
=======
La secuencia de folios es **global por período**, compartida entre todos los almacenes.
Si un almacén genera marbetes en dos momentos distintos, sus folios quedan en
**bloques no consecutivos** porque otros almacenes generaron en medio.
>>>>>>> Stashed changes
...
=======
La validación anterior asumía que todos los folios del rango debían pertenecer
al mismo almacén:
>>>>>>> Stashed changes
...
Esto evita que una impresión normal re-actualice marbetes ya impresos
que caigan dentro del mismo rango de folios.
>>>>>>> Stashed changes
...
| Folios únicos por período | ✅ Secuencia global no modificada |
| Impresión normal solo actualiza GENERADO | ✅ `isReprint=false` filtra por GENERADO |
| Reimpresión solo procesa IMPRESOS | ✅ `isReprint=true` filtra por IMPRESO |
| No imprimir CANCELADOS | ✅ `validateLabelsForPrinting()` los rechaza |
>>>>>>> Stashed changes
...
=======
Después de completar el flujo completo (generar → imprimir → contar), el sistema
permitía volver al paso de generación y crear más marbetes para el mismo
período y almacén. Esto comprometía la integridad de los conteos en curso.
>>>>>>> Stashed changes
...
=======
Se agregó una validación al inicio del método que verifica si ya existen
marbetes en estado `IMPRESO` para ese período y almacén antes de generar:
>>>>>>> Stashed changes
...
=======
|---|---|
| Mismo período, mismo almacén, ya tiene IMPRESOS | ❌ Bloqueado |
| Mismo período, **diferente** almacén | ✅ Permitido |
| Mismo almacén, **diferente** período | ✅ Permitido |
| Mismo período, mismo almacén, solo GENERADOS (sin imprimir) | ✅ Permitido |

### Cambio en frontend requerido

**Ninguno obligatorio.** El backend retorna `400 BAD REQUEST` con mensaje claro.
Opcionalmente el frontend puede deshabilitar el botón "Generar" consultando
si ya existen marbetes impresos para el período/almacén seleccionado.
...
|---|---|
| `LabelsPersistenceAdapter.java` | Eliminada validación incorrecta de rango consecutivo, agregado parámetro `isReprint`, agregado método `existsImpresosForPeriodAndWarehouse()` |
| `LabelGenerationService.java` | Validación al inicio de `generateBatchList()` para bloquear si ya hay IMPRESOS |
| `LabelServiceImpl.java` | Llamadas a `printLabelsRange()` actualizadas con `isReprint=false/true` |
